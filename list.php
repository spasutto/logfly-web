...
<?php
  ...
  for ($i=0; $i<$nbpages; $i++) {
    ...
    $lnpages .= "&nbsp;";
    if ($lnoffset != $offset)
      $lnpages .= "<a href=\"".url_with_parameter("offset", $lnoffset)."\">";
    $lnpages .= ($i+1);
    if ($lnoffset != $offset)
      $lnpages .= "</a>";
  }
  $lnpages .= "<BR>";
	...
	echo "</h1>";
	echo "<h2>Voiles  (".count($vols->voiles).") : </h2><p>".implode(", ", array_map(function($x) { return "<span class=\"ppt_info\"><a href=\"".url_with_parameter("voile", $x->nom, "offset")."\">".$x->nom." (".$x->nombrevols." vols, ".Utils::timeFromSeconds($x->tempsvol, TRUE).")</a></span>"; }, $vols->voiles))."</p>";
	if (!is_array($vols->sites))
	  $vols->sites = [$vols->sites];
	echo "<h2>Sites (".count($vols->sites).") : </h2><p>".implode(", ", array_map(function($x) { return "<span class=\"ppt_info\"><a href=\"".url_with_parameter("site", $x->nom, "offset")."\">".str_replace(" ", "&nbsp;", $x->nom)." (".$x->nombrevols." vols, ".Utils::timeFromSeconds($x->tempsvol, TRUE).")</a></span>"; }, $vols->sites/*$lgfr->getInfoSite()*/))."</p>";
  echo "<h2>Détails : </h2>";
  echo $lnpages;
